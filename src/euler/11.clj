...
(def *array* (int-array [8 2 22 97 38 15 00 40 00 75 4 5 7 78 52 12 50 77 91 8
...
86 56 00 48 35 71 89 07 5 44 44 37 44 60 21 58 51 54 17 58
...
(defn get-coord [x y]
  "Given x y coordinate, returns value from array"
  (let [position (+ x (* y 20))]
        (aget *array* position)))
...
(defn all-product []
  (for [x (int-array (range 20))
        y (int-array (range 20))]
    (list (right-product x y)
        (left-product x y)
        (up-product x y)
        (down-product x y)
        (left-up-product x y)
        (left-down-product x y)
